<template>
  <b-container class="bv-example-row bg-light mb-2 p-2">
    <b-row class="p-23">
      <b-col class="text-left text-bold"><b></b></b-col>
    ...
    <b-row>
      <b-col class="text-right">
        <div v-if="comment.userId === userInfo.userId">
  ...
  methods: {
    ...
    deleteComment() {
      if (confirm("정말로 삭제?")) {
        // 서버로 삭제할 도서평번호를 전달.
        http
          .delete(`/qna/comment/${this.comment.comment_no}`)
          .then(({ data }) => {
            ...
            this.$store.dispatch(
              "getComments",
              `/qna/comment/${this.comment.articleno}`
            );
          });
      }
    },
